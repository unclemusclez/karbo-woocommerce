...
//===========================================================================
function SIKKAWC__render_general_settings_page ()   { SIKKAWC__render_settings_page   ('general'); }
//function SIKKAWC__render_advanced_settings_page ()  { SIKKAWC__render_settings_page   ('advanced'); }
//===========================================================================

//===========================================================================
function SIKKAWC__render_settings_page ($menu_page_name)
{
   $sikkawc_settings = SIKKAWC__get_settings ();
   if (isset ($_POST['button_withdraw']))
      {
      $result = SIKKAWC__withdraw();
echo '
...
';
   ...
   else if (isset ($_POST['button_update_sikkawc_settings']))
      {
      SIKKAWC__update_settings ("", false);
echo <<<HHHH
<div align="center" style="background-color:#FFFFE0;padding:5px;font-size:120%;border: 1px solid #E6DB55;margin:5px;border-radius:3px;">
Settings updated!
</div>
HHHH;
   ...
   else if (isset($_POST['button_reset_sikkawc_settings']))
      {
      SIKKAWC__reset_all_settings (false);
echo <<<HHHH
<div align="center" style="background-color:#FFFFE0;padding:5px;font-size:120%;border: 1px solid #E6DB55;margin:5px;border-radius:3px;">
...
HHHH;
   ...
   else if (isset($_POST['button_reset_partial_sikkawc_settings']))
      {
      SIKKAWC__reset_partial_settings (false);
echo <<<HHHH
<div align="center" style="background-color:#FFFFE0;padding:5px;font-size:120%;border: 1px solid #E6DB55;margin:5px;border-radius:3px;">
...
HHHH;
  ...
  $gateway_valid_for_use = SIKKAWC__is_gateway_valid_for_use($gateway_status_message);
  if (!$gateway_valid_for_use)
  {
    ...
    "Sikka Payment Gateway is NOT operational (try to re-enter and save settings): " . $gateway_status_message .
    '</p>';
  }
  else
  {
    ...
    {
      $address_balance = __("Sikka address is not found in wallet.", 'woocommerce');
    } else {
    ...
    "Sikka Payment Gateway is operational" .
  ...
  if (!$currency_code || $currency_code=='SIKKA')
    $currency_code = 'USD';

  $exchange_rate_message =
    '<p style="border:1px solid #DDD;padding:5px 10px;background-color:#cceeff;">' .
    SIKKAWC__get_exchange_rate_per_Sikka ($currency_code, 'getfirst', true) .
    '</p>';
   ...
   switch ($menu_page_name)
      {
      case 'general'     :
        echo  SIKKAWC__GetPluginNameVersionEdition();
        echo  $gateway_status_message . $exchange_rate_message;
        SIKKAWC__render_general_settings_page_html();
        break;
...
//===========================================================================
function SIKKAWC__render_general_settings_page_html ()
{
  $sikkawc_settings = SIKKAWC__get_settings ();
...
?>
    ...
    <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
      <p class="submit">
        <input type="submit" class="button-primary"    name="button_update_sikkawc_settings"        value="<?php _e('Save Changes') ?>"             />
        <input type="submit" class="button-secondary"  style="color:red;" name="button_reset_partial_sikkawc_settings" value="<?php _e('Reset settings') ?>" onClick="return confirm('Are you sure you want to reset settings on this page?');" />
      </p>
      <table class="form-table">
        ...
        <tr valign="top">
          ...
          <td>
            <input type="hidden" name="delete_db_tables_on_uninstall" value="0" /><input type="checkbox" name="delete_db_tables_on_uninstall" value="1" <?php if ($sikkawc_settings['delete_db_tables_on_uninstall']) echo 'checked="checked"'; ?> />
        ...
        <tr valign="top">
          <th scope="row">Sikka Service Provider:</th>
          <td>
            <select name="service_provider" class="select ">
              <option <?php if ($sikkawc_settings['service_provider'] == 'local_wallet') echo 'selected="selected"'; ?> value="local_wallet">Local wallet (walletd)</option>
            </select>
            <p class="description">
              Please select your Sikka service provider and press [Save changes]. Then fill-in necessary details and press [Save changes] again.
        ...
        <tr valign="top">
          ...
          <td>
            <input type="text" name="confs_num" value="<?php echo $sikkawc_settings['confs_num']; ?>" size="4" />
            <p class="description">
              After a transaction is broadcast to the Sikka network, it may be included in a block that is published
              to the network. When that happens it is said that one <a href="https://en.Sikka.it/wiki/Confirmation"><b>confirmation</b></a> has occurred for the transaction.
        ...
        <tr valign="top">
          ...
          <td>
            <input type="text" name="assigned_address_expires_in_mins" value="<?php echo $sikkawc_settings['assigned_address_expires_in_mins']; ?>" size="4" />
            <p class="description">
        ...
        <tr valign="top">
          ...
          <td>
            <input type="text" name="exchange_multiplier" value="<?php echo $sikkawc_settings['exchange_multiplier']; ?>" size="4" />
            <p class="description">
              Extra multiplier to apply to convert store default currency to Sikka price.
              <br />Example: 1.05 - will add extra 5% to the total price in Sikkas.
              May be useful to compensate for market volatility or for merchant's loss to fees when converting Sikkas to local currency,
              or to encourage customer to use Sikkas for purchases (by setting multiplier to < 1.00 values).
            </p>
          </td>
        </tr>
        ...
        <tr valign="top">
          ...
          <td>
            <input type="hidden" name="autocomplete_paid_orders" value="0" /><input type="checkbox" name="autocomplete_paid_orders" value="1" <?php if ($sikkawc_settings['autocomplete_paid_orders']) echo 'checked="checked"'; ?> />
        ...
        <tr valign="top">
            ...
            <td>
              <select name="enable_soft_cron_job" class="select ">
                <option <?php if ($sikkawc_settings['enable_soft_cron_job'] == '1') echo 'selected="selected"'; ?> value="1">Soft Cron (Wordpress-driven)</option>
                <option <?php if ($sikkawc_settings['enable_soft_cron_job'] != '1') echo 'selected="selected"'; ?> value="0">Hard Cron (Cpanel-driven)</option>
              </select>
              <p class="description">
                <?php if ($sikkawc_settings['enable_soft_cron_job'] != '1') echo '<p style="background-color:#FFC;color:#2A2;"><b>NOTE</b>: Hard Cron job is enabled: make sure to follow instructions below to enable hard cron job at your hosting panel.</p>'; ?>
                Cron job will take care of all regular Sikka payment processing tasks, like checking if payments are made and automatically completing the orders.<br />
                ...
                <?php echo '<tt style="background-color:#FFA;color:#B00;padding:0px 6px;">wget -O /dev/null ' . $g_SIKKAWC__cron_script_url . '?hardcron=1</tt>'; ?>
                <br /><b style="color:red;">NOTE:</b> Cron jobs <b>might not work</b> if your site is password protected with HTTP Basic auth or other methods. This will result in WooCommerce store not seeing received payments (even though funds will arrive correctly to your Sikka addresses).
                ...
                <br />For secure, fast hosting service optimized for wordpress and 100% compatibility with WooCommerce and Sikka we recommend <b><a href="http://hostrum.com/" target="_blank">Hostrum Hosting</a></b>.
              </p>
            </td>
        </tr>
