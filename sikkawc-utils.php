...
//===========================================================================
function SIKKAWC__generate_new_Sikka_payment_id ($sikkawc_settings=false, $order_info)
{
  global $wpdb;
  ...
  $address_request_array['sikka_address'] = $sikka_address;
  $address_request_array['sikka_payment_id'] = $new_sikka_payment_id;
  ...
  $ret_info_array = SIKKAWC__getreceivedbyaddress_info ($address_request_array, $sikkawc_settings);
...
function SIKKAWC_serialize_address_meta ($address_meta_arr)
{
   return SIKKAWC__safe_string_escape(serialize($address_meta_arr));
}
//===========================================================================
...
function SIKKAWC__getreceivedbyaddress_info ($address_request_array, $sikkawc_settings=false)
{
	if (!$sikkawc_settings)
  	$sikkawc_settings = SIKKAWC__get_settings ();

  $sikka_address            = $address_request_array['sikka_address'];
	$sikka_payment_id         = $address_request_array['sikka_payment_id'];
...
//===========================================================================
// Returns:
//    success: number of currency units (dollars, etc...) would take to convert to 1 Sikka, ex: "15.32476".
//    failure: false
//
...
function SIKKAWC__get_exchange_rate_per_Sikka ($currency_code, $rate_retrieval_method = 'getfirst', $get_ticker_string=false)
{
   if ($currency_code == 'SIKKA')
      return "1.00";   // 1:1

	$sikkawc_settings = SIKKAWC__get_settings ();
  $exchange_rate_type = $sikkawc_settings['exchange_rate_type'];
  $exchange_multiplier = $sikkawc_settings['exchange_multiplier'];
  if (!$exchange_multiplier)
	...
	$ticker_string = "<span style='color:#222;'>According to your settings (including multiplier), current calculated rate for 1 Sikka (in {$currency_code})={{{EXCHANGE_RATE}}}</span>";
	...
	$this_currency_info = @$sikkawc_settings['exchange_rates'][$currency_code][$requested_cache_method_type];
	if ($this_currency_info && isset($this_currency_info['time-last-checked']))
	{
	  $delta = $current_time - $this_currency_info['time-last-checked'];
	  if ($delta < (@$sikkawc_settings['cache_exchange_rates_for_minutes'] * 60))
	  {
  ...
  $exchange_rate = SIKKAWC__get_exchange_rate_from_cryptocompare($currency_code, $exchange_rate_type, $sikkawc_settings); 

  // Save new currency exchange rate info in cache
  SIKKAWC__update_exchange_rate_cache ($currency_code, $requested_cache_method_type, $exchange_rate);
...
//===========================================================================
function SIKKAWC__function_not_exists ($fname) { return !function_exists($fname); }
//===========================================================================

//===========================================================================
function SIKKAWC__update_exchange_rate_cache ($currency_code, $requested_cache_method_type, $exchange_rate)
{
  // Save new currency exchange rate info in cache
  $sikkawc_settings = SIKKAWC__get_settings ();   // Re-get settings in case other piece updated something while we were pulling exchange rate API's...
  $sikkawc_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['time-last-checked'] = time();
  $sikkawc_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['exchange_rate'] = $exchange_rate;
  SIKKAWC__update_settings ($sikkawc_settings);
...
function SIKKAWC__get_exchange_rate_from_cryptocompare ($currency_code, $rate_type, $sikkawc_settings)
{
 $source_url = "https://min-api.cryptocompare.com/data/price?fsym=SIKKA&tsyms=" . $currency_code;
 $result = @SIKKAWC__file_get_contents ($source_url, false, $sikkawc_settings['exchange_rate_api_timeout_secs']);
...
*/
function SIKKAWC__file_get_contents ($url, $return_content_on_error=false, $timeout=60, $user_agent=FALSE, $is_post=false, $post_data="")
{
		...
		foreach ($post_data as $k => $v)
			{
				...
				if (is_object($safetied))
					$safetied = SIKKAWC__object_to_array($safetied);
				if (is_array($safetied))
				{
					$safetied = serialize($safetied);
					$safetied = $p . str_replace('=', '_', SIKKAWC__base64_encode($safetied));
...
//===========================================================================
function SIKKAWC__object_to_array ($object)
{
	if (!is_object($object) && !is_array($object))
    return $object;
  return array_map('SIKKAWC__object_to_array', (array) $object);
}
//===========================================================================

//===========================================================================
// Credits: http://www.php.net/manual/en/function.mysql-real-escape-string.php#100854
function SIKKAWC__safe_string_escape ($str="")
{
   $len=strlen($str);
   $escapeCount=0;
   $targetString='';
...
//===========================================================================
// Syntax:
//    SIKKAWC__log_event (__FILE__, __LINE__, "Hi!");
//    SIKKAWC__log_event (__FILE__, __LINE__, "Hi!", "/..");
//    SIKKAWC__log_event (__FILE__, __LINE__, "Hi!", "", "another_log.php");
function SIKKAWC__log_event ($filename, $linenum, $message, $prepend_path="", $log_file_name='__log.php')
{
   ...
   if ($fhandle)
      {
      @fwrite ($fhandle, "\r\n// " . $_SERVER['REMOTE_ADDR'] . '(' . $_SERVER['REMOTE_PORT'] . ')' . ' -> ' . date("Y-m-d, G:i:s T") . "|" . SIKKAWC_VERSION . "/" . SIKKAWC_EDITION . "|$filename($linenum)|: " . $message . $logfile_tail);
      @fclose ($fhandle);
      }
}
//===========================================================================
...
//===========================================================================
function SIKKAWC__send_email ($email_to, $email_from, $subject, $plain_body)
{
   $message = "
   <html>
   <head>
...
//===========================================================================
function SIKKAWC__is_gateway_valid_for_use (&$ret_reason_message=NULL)
{
  $valid = true;
  $sikkawc_settings = SIKKAWC__get_settings ();
...
////   'service_provider'                     =>  'local_wallet',    // 'blockchain_info'
  ...
  //----------------------------------
  // Validate settings
  if ($sikkawc_settings['service_provider']=='local_wallet')
  {         
          $sikkawc_settings = SIKKAWC__get_settings();
...
function SIKKAWC__base64_decode($input)
{
...
function SIKKAWC__base64_encode($data)
{
    ...
    $o1 = SIKKAWC_charCodeAt($data, $i++);
    $o2 = SIKKAWC_charCodeAt($data, $i++);
    $o3 = SIKKAWC_charCodeAt($data, $i++);
    $bits = $o1 << 16 | $o2 << 8 | $o3;
    $h1 = $bits >> 18 & 0x3f;
    $h2 = $bits >> 12 & 0x3f;
    $h3 = $bits >> 6 & 0x3f;
    ...
    $tmp_arr[$ac++] = SIKKAWC_charAt($b64, $h1).SIKKAWC_charAt($b64, $h2).SIKKAWC_charAt($b64, $h3).SIKKAWC_charAt($b64, $h4);
    } while ($i < strlen($data));
    $enc = implode($tmp_arr, '');
    $r = (strlen($data) % 3);
...
function SIKKAWC_charCodeAt($data, $char) {
    return ord(substr($data, $char, 1));
}

function SIKKAWC_charAt($data, $char) {
